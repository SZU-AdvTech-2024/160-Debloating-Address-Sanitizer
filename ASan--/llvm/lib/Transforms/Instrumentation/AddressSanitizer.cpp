...
void AddressSanitizer::MonotonicOptimizeHandler(Loop *L, std::set<Instruction *> &optimized, 
  ...
  dbgs() << F.getName().str() << " " << L->getName().str() << ExitBB->getName().str() << " Checked Instrution:\n";
  Inst->print(dbgs());
  dbgs() << "\n";

  dbgs() << F.getName().str() << " " << L->getName().str() << ExitBB->getName().str() << " ExitBB:\n";
  for(Instruction &Inst1 : *ExitBB) {
    Inst1.print(dbgs());
    dbgs() << "\n";
  }
  ...
  if (initType == SEUnknown) {
    dbgs() << F.getName().str() << " " << L->getName().str() << " getInitValueFromSCEV(..) = SEUnknown\n";
    return;
  }
  ...
  dbgs() << F.getName().str() << " " << L->getName().str() << " Optimization Success\n";
  ...
  if (DT.dominates(Inst, ExitBB)) {
    ...
    dbgs() << F.getName().str() << " " << L->getName().str() << " Instrumentation in ExitBB\n";
  }
